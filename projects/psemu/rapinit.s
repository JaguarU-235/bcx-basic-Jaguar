...
RAPTOR_MT_app_name:			dc.b	'PSXEMU_BETA',0					; Name of Application
							.even
RAPTOR_MT_file_name:		dc.b	'(C)REBOOT_2015',0					; Name of filename to use
							.even
...
; text layer - sprite 0
	...
	dc.w	255,0																; sprite_y			; 16.16 y value to position at
...
;			value						possible values				description															rb+ label to access
	...
	dc.l	304				; 									; Width of sprite in pixels											; * R_sprite_width
	dc.l	200				; 									; Height of sprite in pixels										; * R_sprite_height
	...
	dc.l	304/2			;									; Width of collision box in pixels									; * R_sprite_hbox
	dc.l	200/2			;									; Height of collision box in pixels									; * R_sprite_vbox
	dc.l	ROM_BMP_ANIM1+(304*200*38)	;									; Place in memory where the sprite lives: label from RAPAPP.S		;   R_sprite_gfxbase
	...
	dc.l	is_opaque		; is_trans			is_opaque		; Trans for see-through background, opaque for solid background		; * R_sprite
	dc.l	304*200 		; 									; Size of sprite in bytes - used to find next frame of animation	;   R_sprite_framesz
																...
																; *2 for 16bit
																; *4 for 24bit
	dc.l	304				; 									; Width of a single row of sprite pixels							; * R_sprite_bytewid
	...
	dc.l	ani_once		; ani_rept			ani_once		; Loop theanimation or play a single time							;	R_sprite_animloop
	...
	dc.l	304				;									; Width of sprite (used for long looping sprites).					;   R_sprite_gwidth
																; Enter the x-size and add the following:
																; /8 for 1bit
																; /4 for 2bit
																; /2 for 4bit
...
;			value						possible values				description															rb+ label to access
	...
	dc.l	304/2			;									; Width of collision box in pixels									; * R_sprite_hbox
	dc.l	200/2			;									; Height of collision box in pixels									; * R_sprite_vbox
	dc.l	ROM_BMP_ANIM1	;									; Place in memory where the sprite lives: label from RAPAPP.S		;   R_sprite_gfxbase
	dc.l	8				; 1  2  4  8  16  24				; Sprite graphics type: 1,2,4,8,16 or 24 bit						; * R_sprite_?
	dc.l	is_RGB			; is_RGB			is_cry			; Sprite graphics type. RGB or Atari's CRY format.					; * R_sprite_?
	dc.l	is_opaque		; is_trans			is_opaque		; Trans for see-through background, opaque for solid background		; * R_sprite
	dc.l	304*200 		; 									; Size of sprite in bytes - used to find next frame of animation	;   R_sprite_framesz
	...
	dc.l	304				; 									; Width of a single row of sprite pixels							; * R_sprite_bytewid
																...
																; *2 for 16bit
																; *4 for 24bit
	dc.l	3				;									; Number of screen updates (VB) before the animation advances		;   R_sprite_framedel
	dc.l	38				; 									; Additional number of animation frames after the first				;   R_sprite_maxframe
	dc.l	ani_once		; ani_rept			ani_once		; Loop theanimation or play a single time							;	R_sprite_animloop
	dc.l	edge_wrap		; edge_wrap			R_edge_kill		; What do you want to do if sprite leaves the edge of the screen?	;   R_sprite_wrap
	dc.l	spr_inf			; spr_inf							; Want sprite to stay forever or a number of frames?				;   R_sprite_timer
	dc.l	spr_linear		; spr_linear 						; Use automatic movement above or use a tracking table.				;   R_sprite_track
...
;			value						possible values				description															rb+ label to access
	...
	dc.l	0				; 									; Collision box x offset - pixel distance from centre of sprite		; * R_sprite_coffx
	dc.l	0				; 									; Collision box y offset - pixel distance from centre of sprite		; * R_sprite_coffy
	dc.l	300/2			;									; Width of collision box in pixels									; * R_sprite_hbox
	dc.l	200/2			;									; Height of collision box in pixels									; * R_sprite_vbox
	dc.l	ROM_BMP_ANIM2	;									; Place in memory where the sprite lives: label from RAPAPP.S		;   R_sprite_gfxbase
	dc.l	8				; 1  2  4  8  16  24				; Sprite graphics type: 1,2,4,8,16 or 24 bit						; * R_sprite_?
	dc.l	is_RGB			; is_RGB			is_cry			; Sprite graphics type. RGB or Atari's CRY format.					; * R_sprite_?
	dc.l	is_opaque		; is_trans			is_opaque		; Trans for see-through background, opaque for solid background		; * R_sprite
	dc.l	304*200 		; 									; Size of sprite in bytes - used to find next frame of animation	;   R_sprite_framesz
	...
	dc.l	304				; 									; Width of a single row of sprite pixels							; * R_sprite_bytewid
																...
																; *2 for 16bit
																; *4 for 24bit
	dc.l	10				;									; Number of screen updates (VB) before the animation advances		;   R_sprite_framedel
...
;			value						possible values				description															rb+ label to access
	dc.l	1				; 									; How many sprites like this one do you want in the sprite list?
	dc.l	is_active		; is_active			is_inactive		; Will the sprite be active on screen or inactive?					;   R_sprite_active
	dc.w	56,0				; 									; Put sprite this many pixels from left side of screen.				;   R_sprite_x
	dc.w	255,0				; 									; Put sprite this many pixels down from top of screen.				;   R_sprite_y
	dc.w	0,0				; 									; How many pixels will this sprite move automatically in x?			;   R_sprite_xadd
	dc.w	0,0				; 									; How many pixels will this sprite move automatically in y?			;   R_sprite_yadd
	dc.l	240				; 									; Width of sprite in pixels											; * R_sprite_width
	...
	dc.l	300/2			;									; Width of collision box in pixels									; * R_sprite_hbox
	dc.l	200/2			;									; Height of collision box in pixels									; * R_sprite_vbox
	dc.l	BMP_TR			;									; Place in memory where the sprite lives: label from RAPAPP.S		;   R_sprite_gfxbase
	dc.l	8				; 1  2  4  8  16  24				; Sprite graphics type: 1,2,4,8,16 or 24 bit						; * R_sprite_?
	dc.l	is_RGB			; is_RGB			is_cry			; Sprite graphics type. RGB or Atari's CRY format.					; * R_sprite_?
	dc.l	is_opaque		; is_trans			is_opaque		; Trans for see-through background, opaque for solid background		; * R_sprite
	dc.l	240*240 		; 									; Size of sprite in bytes - used to find next frame of animation	;   R_sprite_framesz
																; Enter the x-size * the y-size and add the following next:
																; /8 for 1bit
																; /4 for 2bit
																; /2 for 4bit
																...
																; *2 for 16bit
																; *4 for 24bit
	dc.l	240				; 									; Width of a single row of sprite pixels							; * R_sprite_bytewid
																; Enter the x-size and add the following:
																; /8 for 1bit
																; /4 for 2bit
																; /2 for 4bit
	...
	dc.l	1				; 									; Hit points before sprite is killed								;   R_sprite_hitpoint
	dc.l	1				; 									; Sprite hit point damage per hit									;   R_sprite_damage
	dc.l	240				;									; Width of sprite (used for long looping sprites).					;   R_sprite_gwidth
																; Enter the x-size and add the following:
																; /8 for 1bit
																; /4 for 2bit
																; /2 for 4bit

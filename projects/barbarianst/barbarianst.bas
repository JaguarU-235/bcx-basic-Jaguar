...
'_______a remake of ATARI ST version____
...
loadclut(strptr(decorsCPC_clut),0,40) ' palette decors
...
'partie = 1: decorS = 4: IA = 9: sorcier = 0: marianna = 0: demo = 0 ' sorcier

' mode vs
'partie = 2: decorS = 1: IA = 0: sorcier = 0: marianna = 0: chronometre = 90: demo = 0
...
DO
    ...
    IF GOTOU = 100 THEN
		...
		rlist[NoSPRthanks].x=-320<<16		
		rlist[NoSPRthanks].y=700<<16
    ...
    IF GOTOU = 32 THEN
			...
			' choix du decor
			rsetobj(NoSPRdecor,R_sprite_gfxbase,(int)strptr(decorsCPC)+ (64000 * decorS ) ) ' choix de la frame dans la planche 32000 = 320*(200*8)/8

			
		'  affichage de l'arbre gauche
		rsetobj(NoSPRarbreG,R_sprite_gfxbase,(int)strptr(arbresGAUCHE)+ 2048* (decorS-1) ) ' choix de la frame dans la planche 2048 = 64*(64*4)/8
		...
		if scoreA = 0 THEN RLOCATE 100,64: RPRINT "00000"
		if scoreB = 0 THEN RLOCATE 188,64: RPRINT "00000"
		...
		IF partie = 1 AND demo = 0 THEN RLOCATE 92,208: RPRINT "THE BATTLE BEGINS "
		IF partie = 1 AND sorcier = 1 THEN RLOCATE 92,208: RPRINT "THE FINAL CONFLICT"
        IF partie = 2 THEN RLOCATE 92,208: RPRINT "  PRACTICE ARENA  "
		IF demo = 1 THEN RLOCATE 92,208: RPRINT "   DEMONSTRATION   " 
...
songnome = 0
		...
		IF IA = 8 THEN loadclut(strptr(paletteB8_clut),7,16) ' IA8
		IF IA = 9 THEN loadclut(strptr(paletteB9_clut),7,16) ' IA9
            ...
            IF xA >= 39 THEN
                ...
                IF partie = 1 THEN
                    ...
                    IF GOTOU = 0 THEN
                        ...
                        IF IA = 4 THEN decorS = 2
                        IF IA = 5 THEN decorS = 1
                        IF IA = 6 THEN decorS = 2
                        IF IA = 7 THEN decorS = 1
                        IF IA = 8 THEN decorS = 3
                        IF IA = 9 THEN decorS = 4
                        IF IA = 10 THEN decorS = 4						
						
                        IF demo = 1 THEN decorS = 3
                    END IF
            ...
            IF joueurA = 95 OR joueurA = 910 THEN
                ...
                IF partie = 1 THEN
                    ...
                    IF GOTOU = 0 AND transitionfinie = 1 THEN
                        ...
                        decorS = 2: GOTOU = 8 ' version ATARI
                    END IF
    ...
    IF GOTOU = 0 AND joueurA = 63 THEN 'cou
        ...
        IF GOTOU = 0 AND temps > reftemps + 20 THEN
            ...
            IF joueurB = 63 AND spriteB = 20 AND distance < 12 THEN
                ...
                IF choixCLING > 4 THEN choixCLING = 1
				IF musiqueLOOP = 0 THEN 
...
'coco
    ...
    IF GOTOU = 0 AND partie = 1 THEN
        ...
        IF GOTOU = 0 THEN
            IF IA = 4 OR IA = 5 OR IA = 7 OR IA = 8 OR IA = 9 THEN
    ...
    IF GOTOU = 0 AND sorcier = 1 THEN
        ...
        ' cache Drax
				...
				rlist[NoSPRcache1].y=96+13<<16

        'cache marianna
        IF marianna = 1 THEN
				rlist[NoSPRcache2].x=198<<16
				rlist[NoSPRcache2].y=96+13<<16
        END IF
    ...
    IF GOTOU = 0 AND partie = 1 THEN RLOCATE 156,64: RPRINT "0" : PRINT "0": RLOCATE 164,64: RPRINTINT IA

    IF GOTOU = 0 AND partie = 2 THEN

        IF chronometre >= 10 THEN RLOCATE 156,64: RPRINTINT chronometre
        IF chronometre < 10 THEN RLOCATE 156,64: RPRINT "0" : PRINT "0": RLOCATE 164,64: RPRINTINT chronometre
	...
	IF GOTOU = 0 THEN
		...
		if scoreA < 0 OR niveauscoreA > 99 THEN scoreA = 0: scoreAactuel = 0: niveauscoreA = 0: RLOCATE 100,64: RPRINT "00000"
		if scoreB < 0 OR niveauscoreB > 99 THEN scoreB = 0: scoreBactuel = 0: niveauscoreB = 0: RLOCATE 188,64: RPRINT "00000"
        ...
        if scoreAactuel != scoreA THEN
            ...
            if scoreA >=1000 THEN
                scoreA = scoreA - 1000: niveauscoreA = niveauscoreA + 1
            END IF
            RLOCATE 116,64: RPRINTINT scoreA
            if scoreA <= 99 THEN RLOCATE 116,64: RPRINT "00": RLOCATE 124,64: RPRINTINT scoreA

            'affichage des dizaines de milliers
           
            if niveauscoreA <= 9 THEN 
				RLOCATE 100,64: RPRINT "0"
				RLOCATE 108,64: RPRINTINT niveauscoreA
			END IF
			
            if niveauscoreA > 9 THEN RLOCATE 100,64: RPRINTINT niveauscoreA
		...
		if scoreBactuel != scoreB THEN
            ...
            if scoreB >=1000 THEN
                    scoreB = scoreB - 1000: niveauscoreB = niveauscoreB + 1
            END IF
            RLOCATE 204,64: RPRINTINT scoreB
            if scoreB <= 99 THEN RLOCATE 204,64: RPRINT "00": RLOCATE 212,64: RPRINTINT scoreB

            'affichage des dizaines de milliers
            
            if niveauscoreB <= 9 THEN 
				RLOCATE 188,64: RPRINT "0"
				RLOCATE 196,64: RPRINTINT niveauscoreB
			END IF
			
            if niveauscoreB > 9 THEN RLOCATE 188,64: RPRINTINT niveauscoreB
...
'END IF
     ...
     ' boulefeu
      if  GOTOU = 9 THEN GOTOU = 0
        ...
        if  jeu = 1 AND sorcier = 1 AND xAttB > xB + 4 THEN 
          ...
          positionX =  ( xAttB - 1 ) * 8: positionY =  ( yAttB +2 ) * 8+8
    ...
    IF GOTOU = 0 THEN
        ...
        IF jeu = 2 THEN 
			...
			rlist[NoSPRthanks].x=48<<16 
			rlist[NoSPRthanks].y=100<<16
		END IF

        IF jeu = 0 AND joueurA = 6660 THEN RLOCATE 88,124: RPRINT "YOUR.END.HAS.COME"
    ...
    IF arret = 1 THEN
		...
		IF jeu = 1 THEN	
				...
				IF GOTOU = 0 AND partie = 1 THEN
					partie = 2: decorS = 1: IA = 0: sorcier = 0: marianna = 0: chronometre = 90 ' vs
					GOTOU = 8
				END IF

				IF GOTOU = 0 AND partie = 2 THEN
					partie = 1: decorS = 3: IA = 4: sorcier = 0: marianna = 0: demo = 1 ' demo
					GOTOU = 8
				END IF
...
' *** DEBUG *****	
'if GOTOU = 0 THEN
	'RLOCATE 0,208: RPRINT "      ": RPRINTINT xA
	'RLOCATE 0,208: RPRINT "      ": RPRINTINT xB
'END IF	

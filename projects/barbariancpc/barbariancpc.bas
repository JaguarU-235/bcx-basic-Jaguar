...
'____a remake of AMSTRAD CPC version____
'___________for ATARI Jaguar____________
'____________by F.L in 2017_____________
...
loadclut(strptr(decorsCPC_clut),0,16) ' palette decors
...
' 1 joueur
'partie = 1: decorS = 4: IA = 0: sorcier = 0: marianna = 0: demo = 0
...
DO
    ...
    IF GOTOU = 100 THEN
		...
		RLOCATE 4,4: RPRINT "      ": RLOCATE 4,12: RPRINT "      "		
		RLOCATE 276,4: RPRINT "      ":	RLOCATE 276,12: RPRINT "      "
	
			
		' affichage du titre.	
		rlist[NoSPRtitre].y=24<<16 		
    ...
    IF GOTOU = 32 THEN
			...
			IF easter = 0 THEN 
				...
				RLOCATE 124,128: RPRINT "          ": RLOCATE 148,136: RPRINT "    "
			END IF
			...
			' choix du decor
			rsetobj(NoSPRdecor,R_sprite_gfxbase,(int)strptr(decorsCPC)+ (32000 * decorS ) ) ' choix de la frame dans la planche 32000 = 320*(200*4)/8
		...
		rlist[NoSPRarbreG].y=136<<16
			
		'  affichage de l'arbre droit
		rsetobj(NoSPRarbreD,R_sprite_gfxbase,(int)strptr(arbresDROIT)+ 1536* (decorS-1) )			
		rlist[NoSPRarbreD].y=136<<16		

		
		' re-afficher les sprites
		rlist[NoSPRspriteA].y=120<<16	'joueurA
		rlist[NoSPRspriteB].y=120<<16	'joueurB		


		' affichage des scores
		if scoreA = 0 THEN RLOCATE 100,48: RPRINT "00000"
		if scoreB = 0 THEN RLOCATE 188,48: RPRINT "00000"
		scoreAactuel = 0' pour re-afficher les scores
		scoreBactuel = 0

		' affichage des ponts de vie
		RLOCATE 4,4: RPRINT "()()()"
		RLOCATE 4,12: RPRINT "()()()"		
		RLOCATE 276,4: RPRINT "()()()"
		RLOCATE 276,12: RPRINT "()()()"

		
         ' messages du bas d'ecran
		 rlist[NoSPRmode].x=155<<16
		 rlist[NoSPRmode].y=216<<16
...
lancerintro = 0
    ...
    IF GOTOU = 0 THEN temps = temps + 1 : delaiMUSIQUE = delaiMUSIQUE + 1
	...
	IF toucheMUSIQUE = 1 AND delaiMUSIQUE > 15 THEN
		IF musiqueLOOP = 0 THEN
			...
			SNDZEROPLAY(1, (void *)(sfx_theme), (sfx_theme_end-sfx_theme)+3 and 0xfffffffc, 46168/9233, Zero_Audio_8bit_muLaw|Zero_Audio_Looping)
		END IF
    END IF
	IF toucheMUSIQUE = 1 AND delaiMUSIQUE > 15 THEN
        ...
        IF joueurB = 95 OR joueurB = 910 THEN
            IF xA >= 39 THEN
                ...
                IF partie = 1 THEN
                    ...
                    ' changement de decor
                    IF GOTOU = 0 THEN
                        IF IA = 0 THEN decorS = 4
                        IF IA = 1 THEN decorS = 4
                        IF IA = 2 THEN decorS = 4
                        IF IA = 3 THEN decorS = 4
                        IF IA = 4 THEN decorS = 4
                        IF IA = 5 THEN decorS = 4
                        IF IA = 6 THEN decorS = 4
    ...
    IF GOTOU = 0 AND entree = 1 THEN
        IF lancerintro = 0 THEN
			lancerintro = 1 
			IF musiqueLOOP = 0 THEN SNDZEROPLAY(1, (void *)(sfx_theme), (sfx_theme_end-sfx_theme)+3 and 0xfffffffc, 46168/9233, Zero_Audio_8bit_muLaw|Zero_Audio_Looping)
		END IF
    ...
    IF GOTOU = 0 AND joueurA = 9901 THEN 'recule1R
        ...
        xFA = xA: xTA = xA: xMA = xA: xGA = xA
        IF GOTOU = 0 AND attaqueA = 1 AND demo = 0 AND sortieA = 0 THEN joueurA = 9140: Aoccupe = 1: reftemps = temps: GOTOU = 26
        IF GOTOU = 0 AND temps > reftemps + 9 THEN xA = xA + 1: spriteA = 6: spritereculeA = 2
        GOTOU = 1
    END IF
    ...
    IF GOTOU = 0 AND joueurA = 9902 THEN 'recule2r
        ...
        spriteA = 6
        IF GOTOU = 0 AND attaqueA = 1 AND demo = 0 AND sortieA = 0 THEN joueurA = 9140: Aoccupe = 1: reftemps = temps: GOTOU = 26
        IF GOTOU = 0 AND temps > reftemps + 18 THEN xA = xA + 1: spriteA = 6: spritereculeA = 3
        GOTOU = 1
    END IF
    ...
    IF GOTOU = 0 AND joueurA = 63 THEN 'cou
        ...
        IF GOTOU = 0 AND temps > reftemps + 20 THEN
            ...
            IF joueurB = 63 AND spriteB = 20 AND distance < 12 THEN
				...
				IF musiqueLOOP = 0 THEN SNDZEROPLAY(1, strptr(sfx_cling), (strptr(sfx_cling_end)-strptr(sfx_cling)+3) and 0xfffffffc, 46168/11542, Zero_Audio_8bit_muLaw|0)
...
'coco
    ...
    IF GOTOU = 0 AND joueurA = 620 THEN 'touche
        ...
        IF GOTOU = 0 AND vieA <= 0 THEN
			IF musiqueLOOP = 0 THEN SNDZEROPLAY(1, strptr(sfx_mort), (strptr(sfx_mort_end)-strptr(sfx_mort)+3) and 0xfffffffc, 46168/11542, Zero_Audio_8bit_muLaw|0)
			serpentA = 0: sangA = 1
    ...
    IF GOTOU = 0 AND joueurA = 6201 THEN 'touche1
        ...
        IF GOTOU = 0 AND temps > reftemps + 2 THEN
			...
			IF musiqueLOOP = 0 THEN SNDZEROPLAY(1, strptr(sfx_touche), (strptr(sfx_touche_end)-strptr(sfx_touche)+3) and 0xfffffffc, 46168/11542, Zero_Audio_8bit_muLaw|0)
    ...
    IF GOTOU = 0 AND joueurA = 6191 THEN 'tombe1
        ...
        IF GOTOU = 0 AND temps > reftemps + 2 THEN
            ...
            IF joueurB = 611 AND musiqueLOOP = 0 THEN SNDZEROPLAY(1, strptr(sfx_pied), (strptr(sfx_pied_end)-strptr(sfx_pied)+3) and 0xfffffffc, 46168/11542, Zero_Audio_8bit_muLaw|0)
            'IF joueurB = 617 THEN _SNDSTOP tombe: _SNDPLAY coupdetete
            GOTOU = 1
        END IF
    ...
    IF GOTOU = 0 AND joueurA = 9191 THEN 'tombe1r
        ...
        IF GOTOU = 0 AND temps > reftemps + 2 THEN
            ...
            '_SNDPLAY tombe
			IF musiqueLOOP = 0 THEN SNDZEROPLAY(1, strptr(sfx_tete), (strptr(sfx_tete_end)-strptr(sfx_tete)+3) and 0xfffffffc, 46168/9233, Zero_Audio_8bit_muLaw|0)
            IF joueurB = 911 AND musiqueLOOP = 0 THEN SNDZEROPLAY(1, strptr(sfx_pied), (strptr(sfx_pied_end)-strptr(sfx_pied)+3) and 0xfffffffc, 46168/11542, Zero_Audio_8bit_muLaw|0)
            'IF joueurB = 917 THEN _SNDSTOP tombe: _SNDPLAY coupdetete
            GOTOU = 1
        END IF
        IF GOTOU = 0 AND temps >= reftemps THEN spriteA = 45: GOTOU = 1
    END IF
...
'coco
    ...
    IF GOTOU = 0 AND joueurA = 65 THEN 'mortdecap
        ...
        IF GOTOU = 0 AND temps >= reftemps THEN
			...
			'_SNDPLAY mortdecap
			IF musiqueLOOP = 0 THEN SNDZEROPLAY(1, strptr(sfx_decapite), (strptr(sfx_decapite_end)-strptr(sfx_decapite)+3) and 0xfffffffc, 46168/11542, Zero_Audio_8bit_muLaw|0)
        END IF
...
'coco
    ...
    IF GOTOU = 0 AND partie = 1 THEN
        ...
        IF GOTOU = 0 THEN
            IF IA = 4 OR IA = 5 OR IA = 6 OR IA = 7 THEN
    ...
    IF GOTOU = 0 AND joueurB = 6191 THEN 'tombe1
        ...
        IF GOTOU = 0 AND temps > reftempsB + 2 THEN
            ...
            '_SNDPLAY tombe:
			IF musiqueLOOP = 0 THEN SNDZEROPLAY(1, strptr(sfx_tete), (strptr(sfx_tete_end)-strptr(sfx_tete)+3) and 0xfffffffc, 46168/9233, Zero_Audio_8bit_muLaw|0)
            IF joueurA = 611 AND musiqueLOOP = 0 THEN SNDZEROPLAY(1, strptr(sfx_pied), (strptr(sfx_pied_end)-strptr(sfx_pied)+3) and 0xfffffffc, 46168/11542, Zero_Audio_8bit_muLaw|0)
            'IF joueurA = 617 THEN _SNDSTOP tombe: _SNDPLAY coupdetete
            GOTOU = 2
        END IF
    ...
    IF GOTOU = 0 AND joueurB = 9191 THEN 'tombe1R
        ...
        IF GOTOU = 0 AND temps > reftempsB + 2 THEN
            ...
            '_SNDPLAY tombe
			IF musiqueLOOP = 0 THEN SNDZEROPLAY(1, strptr(sfx_tete), (strptr(sfx_tete_end)-strptr(sfx_tete)+3) and 0xfffffffc, 46168/9233, Zero_Audio_8bit_muLaw|0)
            IF joueurA = 911 AND musiqueLOOP = 0 THEN SNDZEROPLAY(1, strptr(sfx_pied), (strptr(sfx_pied_end)-strptr(sfx_pied)+3) and 0xfffffffc, 46168/11542, Zero_Audio_8bit_muLaw|0)
            'IF joueurA = 917 THEN _SNDSTOP tombe: _SNDPLAY coupdetete
            GOTOU = 2
        END IF
        IF GOTOU = 0 AND temps >= reftempsB THEN spriteB = 45: GOTOU = 2
    END IF
    ...
    IF gnome = 1 THEN
           ...
           if GOTOU = 0 AND temps > reftemps + 65 THEN
                numerognome = 1:reftemps = temps: GOTOU = 31
                'if musiqueLOOP = 0 THEN
                        '#asm
                        'ldw_xy 	#_son_gnome
                        'lda		#00
           ...
           if GOTOU = 0 AND temps > reftemps + 60 THEN numerognome = 4: GOTOU = 31
           if GOTOU = 0 AND temps > reftemps + 56 THEN GOTOU = 31
           if GOTOU = 0 AND temps > reftemps + 55 THEN numerognome = 3: xGNOME = xGNOME + 1: GOTOU = 31
           if GOTOU = 0 AND temps > reftemps + 50 THEN numerognome = 2: GOTOU = 31
           if GOTOU = 0 AND temps > reftemps + 46 THEN GOTOU = 31
           if GOTOU = 0 AND temps > reftemps + 45 THEN numerognome = 1: xGNOME = xGNOME + 1: GOTOU = 31

           if GOTOU = 0 AND temps > reftemps + 40 THEN numerognome = 4: GOTOU = 31
           if GOTOU = 0 AND temps > reftemps + 36 THEN GOTOU = 31
           if GOTOU = 0 AND temps > reftemps + 35 THEN numerognome = 3: xGNOME = xGNOME + 1: GOTOU = 31
           if GOTOU = 0 AND temps > reftemps + 30 THEN  numerognome = 2: GOTOU = 31
           if GOTOU = 0 AND temps > reftemps + 26 THEN GOTOU = 31
           if GOTOU = 0 AND temps > reftemps + 25 THEN numerognome = 1: xGNOME = xGNOME + 1: GOTOU = 31

           if GOTOU = 0 AND temps > reftemps + 20 THEN numerognome = 4: GOTOU = 31
           if GOTOU = 0 AND temps > reftemps + 16 THEN GOTOU = 31
           if GOTOU = 0 AND temps > reftemps + 15 THEN numerognome = 3: xGNOME = xGNOME + 1: GOTOU = 31
           if GOTOU = 0 AND temps > reftemps + 10 THEN  numerognome = 2: GOTOU = 31
           if GOTOU = 0 AND temps > reftemps + 6 THEN GOTOU = 31
           if GOTOU = 0 AND temps > reftemps + 5 THEN numerognome = 1: xGNOME = xGNOME + 1: GOTOU = 31
           if GOTOU = 0 AND temps >= reftemps THEN numerognome = 1: GOTOU = 31
    ...
    IF GOTOU = 0 AND sorcier = 1 THEN
        ...
        ' cache Drax
				rlist[NoSPRcache1].x=126<<16
				rlist[NoSPRcache1].y=96<<16

        'cache marianna
        IF marianna = 1 THEN
				rlist[NoSPRcache2].x=198<<16
				rlist[NoSPRcache2].y=96<<16
        END IF
    ...
    IF GOTOU = 0 AND partie = 1 THEN RLOCATE 156,48: RPRINT "0" : PRINT "0": RLOCATE 164,48: RPRINTINT IA

    IF GOTOU = 0 AND partie = 2 THEN

        IF chronometre >= 10 THEN RLOCATE 156,48: RPRINTINT chronometre
        IF chronometre < 10 THEN RLOCATE 156,48: RPRINT "0" : PRINT "0": RLOCATE 164,48: RPRINTINT chronometre
	...
	IF GOTOU = 0 THEN
		...
		if scoreA < 0 OR niveauscoreA > 99 THEN scoreA = 0: scoreAactuel = 0: niveauscoreA = 0: RLOCATE 100,48: RPRINT "00000"
		if scoreB < 0 OR niveauscoreB > 99 THEN scoreB = 0: scoreBactuel = 0: niveauscoreB = 0: RLOCATE 188,48: RPRINT "00000"
        ...
        if scoreAactuel != scoreA THEN
            ...
            if scoreA >=1000 THEN
                scoreA = scoreA - 1000: niveauscoreA = niveauscoreA + 1
            END IF
            RLOCATE 116,48: RPRINTINT scoreA
            if scoreA <= 99 THEN RLOCATE 116,48: RPRINT "00": RLOCATE 124,48: RPRINTINT scoreA

            'affichage des dizaines de milliers
           
            if niveauscoreA <= 9 THEN 
				RLOCATE 100,48: RPRINT "0"
				RLOCATE 108,48: RPRINTINT niveauscoreA
			END IF
			
            if niveauscoreA > 9 THEN RLOCATE 100,48: RPRINTINT niveauscoreA
		...
		if scoreBactuel != scoreB THEN
            ...
            if scoreB >=1000 THEN
                    scoreB = scoreB - 1000: niveauscoreB = niveauscoreB + 1
            END IF
            RLOCATE 204,48: RPRINTINT scoreB
            if scoreB <= 99 THEN RLOCATE 204,48: RPRINT "00": RLOCATE 212,48: RPRINTINT scoreB

            'affichage des dizaines de milliers
            
            if niveauscoreB <= 9 THEN 
				RLOCATE 188,48: RPRINT "0"
				RLOCATE 196,48: RPRINTINT niveauscoreB
			END IF
			
            if niveauscoreB > 9 THEN RLOCATE 188,48: RPRINTINT niveauscoreB
    ...
    '                                  effacement des points de vie
        ...
        IF vieactuelleA != vieA THEN
            ...
            vieactuelleA = vieA
			IF vieA = 11 THEN RLOCATE 44,12: RPRINT " "
			IF vieA = 10 THEN RLOCATE 36,12: RPRINT " "
			IF vieA = 9 THEN RLOCATE 28,12: RPRINT " "
			IF vieA = 8 THEN RLOCATE 20,12: RPRINT " "
			IF vieA = 7 THEN RLOCATE 12,12: RPRINT " "
			IF vieA = 6 THEN RLOCATE 4,12: RPRINT " "
			IF vieA = 5 THEN RLOCATE 44,4: RPRINT " "
			IF vieA = 4 THEN RLOCATE 36,4: RPRINT " "
			IF vieA = 3 THEN RLOCATE 28,4: RPRINT " "
			IF vieA = 2 THEN RLOCATE 20,4: RPRINT " "
			IF vieA = 1 THEN RLOCATE 12,4: RPRINT " "
			IF vieA = 0 THEN RLOCATE 4,4: RPRINT " "
		...
		IF vieactuelleB != vieB THEN
            ...
            vieactuelleB = vieB
			IF vieB = 11 THEN RLOCATE 316,12: RPRINT " "
			IF vieB = 10 THEN RLOCATE 308,12: RPRINT " "
			IF vieB = 9 THEN RLOCATE 300,12: RPRINT " "
			IF vieB = 8 THEN RLOCATE 292,12: RPRINT " "
			IF vieB = 7 THEN RLOCATE 284,12: RPRINT " "
			IF vieB = 6 THEN RLOCATE 276,12: RPRINT " "
			IF vieB = 5 THEN RLOCATE 316,4: RPRINT " "
			IF vieB = 4 THEN RLOCATE 308,4: RPRINT " "
			IF vieB = 3 THEN RLOCATE 300,4: RPRINT " "
			IF vieB = 2 THEN RLOCATE 292,4: RPRINT " "
			IF vieB = 1 THEN RLOCATE 284,4: RPRINT " "
			IF vieB = 0 THEN RLOCATE 276,4: RPRINT " "
		END IF


		IF easter = 1 THEN
			RLOCATE 124,128: RPRINT "FL.VERSION": RLOCATE 148,136: RPRINT "2018"
		END IF
    ...
    IF GOTOU = 0 THEN
        ...
        IF joueurA = 610 OR joueurA = 910 OR joueurA = 65 OR joueurA = 95 THEN
			...
			rlist[NoSPRspriteAmort].y=120<<16
...
'END IF
    ...
    IF GOTOU = 0 AND serpentA = 1 THEN
		...
		rlist[NoSPRserpentG].y=40<<16		
			
		IF GOTOU = 0 AND temps > refserpentA + 24 THEN
			rsetobj(NoSPRserpentG,R_sprite_gfxbase,(int)strptr(SPRserpent)+ (512 * 0 ) ) ' choix de la frame dans la planche 512 = 32*(32*4)/8
			serpentA = 0 : GOTOU = 10
		END IF
		IF GOTOU = 0 AND temps > refserpentA + 16 THEN
			rsetobj(NoSPRserpentG,R_sprite_gfxbase,(int)strptr(SPRserpent)+ (512 * 1 ) )
			GOTOU = 10
		END IF
		IF GOTOU = 0 AND temps > refserpentA + 8 THEN
			rsetobj(NoSPRserpentG,R_sprite_gfxbase,(int)strptr(SPRserpent)+ (512 * 2 ) )
			GOTOU = 10
		END IF
		IF GOTOU = 0 AND temps >= refserpentA THEN
			rsetobj(NoSPRserpentG,R_sprite_gfxbase,(int)strptr(SPRserpent)+ (512 * 1 ) )
			GOTOU = 10
		END IF
    ...
    IF GOTOU = 0 AND serpentB = 1 THEN
		...
		rlist[NoSPRserpentD].y=40<<16
	
		IF GOTOU = 0 AND temps > refserpentB + 24 THEN 
			rsetobj(NoSPRserpentD,R_sprite_gfxbase,(int)strptr(SPRserpent)+ (512 * 0 ) )
			serpentB = 0 : GOTOU = 17
		END IF
		IF GOTOU = 0 AND temps > refserpentB + 16 THEN
			rsetobj(NoSPRserpentD,R_sprite_gfxbase,(int)strptr(SPRserpent)+ (512 * 1 ) )			
			GOTOU = 17
		END IF
		IF GOTOU = 0 AND temps > refserpentB + 8 THEN
			rsetobj(NoSPRserpentD,R_sprite_gfxbase,(int)strptr(SPRserpent)+ (512 * 2 ) )			
			GOTOU = 17
		END IF
		IF GOTOU = 0 AND temps >= refserpentB THEN
			rsetobj(NoSPRserpentD,R_sprite_gfxbase,(int)strptr(SPRserpent)+ (512 * 1 ) )			
			GOTOU = 17
		END IF
              ...
              '  affichage des tetes */
                  ...
                  xSPRt = (xSPRt + 1) * 8 '  laisser en x8 pour le football ,) */
                   ...
                   positionX = xSPRt : positionY = ySPRt + 24
					...
					rlist[NoSPRtetes].y=positionY<<16


                   ' ombre de la tete
                   positionX = xSPRt: positionY = 182
				   rlist[NoSPRombre].x=positionX<<16
				   rlist[NoSPRombre].y=positionY<<16
    ...
    IF GOTOU = 0 AND gnome = 1 THEN 
		...
		rlist[NoSPRgnome].y=168<<16	'gnome
     ...
     ' boulefeu
      if  GOTOU = 9 THEN GOTOU = 0
        ...
        if  jeu = 1 AND sorcier = 1 AND xAttB > xB + 4 THEN 
          ...
          positionX =  ( xAttB - 1 ) * 8: positionY =  ( yAttB +2 ) * 8
    ...
    IF GOTOU = 0 THEN
        ...
        IF jeu = 2 THEN RLOCATE 112,184: RPRINT "THANKS.BIG.BOY"

        IF jeu = 0 AND joueurA = 6660 THEN RLOCATE 88,112: RPRINT "YOUR.END.HAS.COME"
    ...
    IF arret = 1 THEN
        ...
        IF jeu = 2 OR jeu = 0 THEN		
            ...
            partie = 1: decorS = 4: IA = 0: sorcier = 0: marianna = 0
            GOTOU = 8
        END IF
...
'RLOCATE 0,208: RPRINT "BETA.060"
'RLOCATE 100,0: RPRINT "   ":RPRINTINT GOTOU
'RLOCATE 160,0: RPRINT "   ":RPRINTINT loading

' *** DEBUG *****	
if GOTOU = 0 THEN
	'RLOCATE 0,192: RPRINT "      ": RPRINTINT xA
	'RLOCATE 0,200: RPRINT "      ": RPRINTINT xB
END IF	



' verifier couleur ecritures du titre 
' faire le titre en dernier sprite

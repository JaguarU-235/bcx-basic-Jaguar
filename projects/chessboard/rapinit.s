...
;	----------------------------------------------------------------------------------------------------------
	...
	dc.l	384																; sprite_width		; width of sprite (in pixels)
	dc.l	288																; sprite_height		; height of sprite (in pixels)
	...
	dc.l	BMP_CHESSBOARD													; sprite_gfxbase	; start of bitmap data
	...
	dc.l	384*288*2														; sprite_framesz	; size per frame in bytes of sprite data
	dc.l	384*2															; sprite_bytewid	; width in bytes of one line of sprite data
	...
	dc.l	0																; sprite_hbox		; width of collision box
	dc.l	0																; sprite_vbox		; height of collision box
	dc.l	BMP_SPIDEY														; sprite_gfxbase	; start of bitmap data
	dc.l	4																; (BIT DEPTH)		; bitmap depth (1/2/4/8/16/24)
	dc.l	is_RGB															; (CRY/RGB)			; bitmap GFX type
	dc.l	is_trans														; (TRANSPARENCY)	; bitmap TRANS flag
	dc.l	80*128/2														; sprite_framesz	; size per frame in bytes of sprite data
	...
	dc.l	75																; sprite_maxframe	; number of frames in animation chain
	dc.l	ani_rept														; sprite_animloop	; repeat or play once
	dc.l	edge_wrap														; sprite_wrap		; wrap on screen exit, or remove
	dc.l	spr_inf															; sprite_timer		; frames sprite is active for (or spr_inf)
	dc.l	spr_linear														; sprite_track		; use 16.16 xadd/yadd or point to 16.16 x/y table
	dc.l	0																; sprite_tracktop	; pointer to loop point in track table (if used)
	dc.l	spr_scale														; sprite_scaled		; flag for scaleable object
	dc.l	%00001000														; sprite_scale_x	; x scale factor (if scaled)
	...
	dc.l	1																; (REPEAT COUNTER) 	; Create this many objects of this type (or 1 for a single object)
	dc.l	is_inactive														; sprite_active		; sprite active flag
	dc.w	304-60,0															; sprite_x			; 16.16 x value to position at
	dc.w	192,0															; sprite_y			; 16.16 y value to position at
	dc.w	0,0																; sprite_xadd		; 16.16 x addition for sprite movement
	dc.w	0,0																; sprite_yadd		; 16.16 y addition for sprite movement
	dc.l	80																; sprite_width		; width of sprite (in pixels)
	...
	dc.l	0																; sprite_tracktop	; pointer to loop point in track table (if used)
	dc.l	spr_scale														; sprite_scaled		; flag for scaleable object
	dc.l	%00010000														; sprite_scale_x	; x scale factor (if scaled)
	dc.l	%00010000														; sprite_scale_y	; y scale factor (if scaled)
	dc.l	-1																; sprite_was_hit	; initially flagged as not hit
	dc.l	0																; sprite_CLUT		; no_CLUT (8/16/24 bit) or CLUT (1/2/4 bit)
	dc.l	cant_hit														; sprite_colchk		; if sprite can collide with another
	dc.l	cd_remove														; sprite_remhit		; flag to remove (or keep) on collision
	...
	dc.l	1																; (REPEAT COUNTER) 	; Create this many objects of this type (or 1 for a single object)
	dc.l	is_inactive														; sprite_active		; sprite active flag
	dc.w	274-40,0															; sprite_x			; 16.16 x value to position at
	dc.w	168,0															; sprite_y			; 16.16 y value to position at
	dc.w	0,0															; sprite_xadd		; 16.16 x addition for sprite movement
	dc.w	0,0																; sprite_yadd		; 16.16 y addition for sprite movement
	dc.l	80																; sprite_width		; width of sprite (in pixels)
	...
	dc.l	1																; (REPEAT COUNTER) 	; Create this many objects of this type (or 1 for a single object)
	dc.l	is_inactive														; sprite_active		; sprite active flag
	dc.w	52,0															; sprite_x			; 16.16 x value to position at
	dc.w	112,0															; sprite_y			; 16.16 y value to position at
	dc.w	0,0																; sprite_xadd		; 16.16 x addition for sprite movement
	dc.w	0,0															; sprite_yadd		; 16.16 y addition for sprite movement
	dc.l	80																; sprite_width		; width of sprite (in pixels)
	dc.l	128																; sprite_height		; height of sprite (in pixels)
	...
	dc.l	%01000000														; sprite_scale_x	; x scale factor (if scaled)
	dc.l	%01000000														; sprite_scale_y	; y scale factor (if scaled)
	dc.l	-1																; sprite_was_hit	; initially flagged as not hit
	dc.l	0																; sprite_CLUT		; no_CLUT (8/16/24 bit) or CLUT (1/2/4 bit)
	dc.l	cant_hit														; sprite_colchk		; if sprite can collide with another
	dc.l	cd_remove														; sprite_remhit		; flag to remove (or keep) on collision
	...
	dc.l	1																; (REPEAT COUNTER) 	; Create this many objects of this type (or 1 for a single object)
	dc.l	is_inactive														; sprite_active		; sprite active flag
	dc.w	224-0,0															; sprite_x			; 16.16 x value to position at
	dc.w	112,0															; sprite_y			; 16.16 y value to position at
	dc.w	0,0																; sprite_xadd		; 16.16 x addition for sprite movement
	dc.w	0,0															; sprite_yadd		; 16.16 y addition for sprite movement
	dc.l	80																; sprite_width		; width of sprite (in pixels)
	...
	dc.l	1																; (REPEAT COUNTER) 	; Create this many objects of this type (or 1 for a single object)
	dc.l	is_inactive														; sprite_active		; sprite active flag
	dc.w	72,0															; sprite_x			; 16.16 x value to position at
	...
	dc.l	0																; sprite_tracktop	; pointer to loop point in track table (if used)
	dc.l	spr_scale														; sprite_scaled		; flag for scaleable object
	dc.l	%00110000														; sprite_scale_x	; x scale factor (if scaled)
	dc.l	%00110000														; sprite_scale_y	; y scale factor (if scaled)
	dc.l	-1																; sprite_was_hit	; initially flagged as not hit
	dc.l	0																; sprite_CLUT		; no_CLUT (8/16/24 bit) or CLUT (1/2/4 bit)
	dc.l	cant_hit														; sprite_colchk		; if sprite can collide with another
	dc.l	cd_remove														; sprite_remhit		; flag to remove (or keep) on collision
	...
	dc.l	1																; (REPEAT COUNTER) 	; Create this many objects of this type (or 1 for a single object)
	dc.l	is_inactive														; sprite_active		; sprite active flag
	dc.w	164+40,0															; sprite_x			; 16.16 x value to position at
	dc.w	56,0															; sprite_y			; 16.16 y value to position at
	dc.w	0,0																; sprite_xadd		; 16.16 x addition for sprite movement
	dc.w	0,0															; sprite_yadd		; 16.16 y addition for sprite movement
	dc.l	80																; sprite_width		; width of sprite (in pixels)
	...
	dc.l	0																; sprite_hbox		; width of collision box
	dc.l	0																; sprite_vbox		; height of collision box
	dc.l	BMP_PRESS_A														; sprite_gfxbase	; start of bitmap data
	dc.l	4																; (BIT DEPTH)		; bitmap depth (1/2/4/8/16/24)
	dc.l	is_RGB															; (CRY/RGB)			; bitmap GFX type
	dc.l	is_trans														; (TRANSPARENCY)	; bitmap TRANS flag
	dc.l	352*256/2														; sprite_framesz	; size per frame in bytes of sprite data
	...
	dc.l	is_active														; sprite_active		; sprite active flag
	dc.w	0,0															; sprite_x			; 16.16 x value to position at
	dc.w	252,0															; sprite_y			; 16.16 y value to position at
	dc.w	0,0																; sprite_xadd		; 16.16 x addition for sprite movement
	dc.w	0,0																; sprite_yadd		; 16.16 y addition for sprite movement
	dc.l	352																; sprite_width		; width of sprite (in pixels)
	dc.l	256																; sprite_height		; height of sprite (in pixels)
	...
	dc.l	0																; sprite_hbox		; width of collision box
	dc.l	0																; sprite_vbox		; height of collision box
	dc.l	BMP_WIN														; sprite_gfxbase	; start of bitmap data
	dc.l	4																; (BIT DEPTH)		; bitmap depth (1/2/4/8/16/24)
	dc.l	is_RGB															; (CRY/RGB)			; bitmap GFX type
	dc.l	is_trans														; (TRANSPARENCY)	; bitmap TRANS flag
	dc.l	352*256/2														; sprite_framesz	; size per frame in bytes of sprite data

...
'____a remake of AMSTRAD C64 version____
...
loadclut(strptr(decorsC64_clut),0,16) ' palette decors
...
loadclut(strptr(paletteserpentD_clut),10,16) ' serpent droit
...
DO
    ...
    IF GOTOU = 32 THEN
		...
		RLOCATE 32,71: RPRINT "                                 "
		RLOCATE 32,79: RPRINT "                                 "
		RLOCATE 32,87: RPRINT "                                 "
		RLOCATE 32,95: RPRINT "                                 "
		RLOCATE 32,103: RPRINT "                                 "
		RLOCATE 32,111: RPRINT "                                 "
		RLOCATE 32,119: RPRINT "                                 "
		RLOCATE 32,127: RPRINT "                                 "
		RLOCATE 32,135: RPRINT "                                 "
		RLOCATE 32,143: RPRINT "                                 "
		RLOCATE 32,151: RPRINT "                                 "
		RLOCATE 32,159: RPRINT "                                 "
		RLOCATE 32,167: RPRINT "                                 "
		RLOCATE 32,175: RPRINT "                                 "
		RLOCATE 32,183: RPRINT "                                 "		
			...
			' choix du decor
			rsetobj(NoSPRdecor,R_sprite_gfxbase,(int)strptr(decorsC64)+ (32000 * decorS ) ) ' choix de la frame dans la planche 32000 = 320*(200*4)/8
...
sonrigole = 0
    ...
    IF GOTOU = 0 AND marianna = 1 THEN
            ...
            IF xA <= 1 THEN sortieA = 0: xB = 33: entree = 1 

            IF sortieA = 0 AND  xA >= 25 THEN

                    joueurA = 777: xA = 25
                    Aoccupe = 1: reftemps = temps
                    GOTOU = 26 ' ex 25 */
            END IF
    ...
    IF GOTOU = 0 AND sorcier = 1 THEN
           IF xA >= 28 THEN
              ...
              sortieA = 1: joueurA = 0: Aoccupe = 0 ' faire sortir A */

              Boccupe = 1
              joueurB = 64: xB = 39: marianna = 1: sorcier = 2 ' sorcier = mort */
              ' xAttB=40 */
              GOTOU = 20 ' gestionB */
          END IF
          GOTOU = 23 ' degats */
    ...
    IF GOTOU = 0 AND entree = 1 THEN
		IF lancerintro = 0 THEN
			lancerintro = 1 
			IF musiqueLOOP = 0 THEN 
				SNDZEROPLAY(1, strptr(sfx_intro), (strptr(sfx_intro_end)-strptr(sfx_intro)+3) and 0xfffffffc, 46168/9233, Zero_Audio_8bit_muLaw|0)
			END IF
		END IF
...
'                         ************************************** */
        ...
        IF GOTOU = 0 AND joueurA = 777 THEN ' victoire */

            IF temps > reftemps + 45 THEN
                spriteA = 41: joueurA = 770 ' joueurA fini */
                jeu = 2: GOTOU = 31 ' jeu fini */
            END IF
            IF GOTOU = 0 AND temps > reftemps + 80 THEN spriteA = 41: GOTOU = 31  ' vainqueur2 */
            IF GOTOU = 0 AND temps > reftemps + 30 THEN spriteA = 42: GOTOU = 31  ' vainqueur3 */
            IF GOTOU = 0 AND temps > reftemps + 20 THEN spriteA = 41: GOTOU = 31  ' vainqueur2 */
            IF GOTOU = 0 AND temps > reftemps + 10 THEN spriteA = 40: GOTOU = 31  ' vainqueur1 */
            IF GOTOU = 0 AND temps > reftemps THEN spriteA = 17: GOTOU = 31  ' protegeH */
...
'coco
    ...
    IF transition = 1 THEN
		...
		IF demo = 0 AND temps > reftemps THEN RLOCATE 40,71: RPRINT "_______________________________"
		IF demo = 0 AND temps > reftemps + 4 THEN RLOCATE 40,79: RPRINT "_______________________________"
		IF demo = 0 AND temps > reftemps + 8 THEN RLOCATE 40,87: RPRINT "_______________________________"
		IF demo = 0 AND temps > reftemps + 12 THEN RLOCATE 40,95: RPRINT "_______________________________"
		IF demo = 0 AND temps > reftemps + 16 THEN RLOCATE 40,103: RPRINT "_______________________________"
		IF demo = 0 AND temps > reftemps + 20 THEN RLOCATE 40,111: RPRINT "_______________________________"
		IF demo = 0 AND temps > reftemps + 24 THEN RLOCATE 40,119: RPRINT "_______________________________"
		IF demo = 0 AND temps > reftemps + 28 THEN RLOCATE 40,127: RPRINT "_______________________________"	
		IF demo = 0 AND temps > reftemps + 32 THEN RLOCATE 40,135: RPRINT "_______________________________"	
		IF demo = 0 AND temps > reftemps + 36 THEN RLOCATE 40,143: RPRINT "_______________________________"
		IF demo = 0 AND temps > reftemps + 40 THEN RLOCATE 40,151: RPRINT "_______________________________"
		IF demo = 0 AND temps > reftemps + 44 THEN RLOCATE 40,159: RPRINT "_______________________________"
		IF demo = 0 AND temps > reftemps + 48 THEN RLOCATE 40,167: RPRINT "_______________________________"
		IF demo = 0 AND temps > reftemps + 52 THEN RLOCATE 40,175: RPRINT "_______________________________"
		IF demo = 0 AND temps > reftemps + 56 THEN RLOCATE 40,183: RPRINT "_______________________________"
		'IF demo = 0 AND temps > reftemps + 60 THEN RLOCATE 40,191: RPRINT "______________________________"
...
'END IF
    ...
    IF GOTOU = 0 AND serpentA = 1 THEN
		...
		rlist[NoSPRserpentG].x=19<<16
		rlist[NoSPRserpentG].y=40+8<<16		
    ...
    IF GOTOU = 0 AND serpentB = 1 THEN
		...
		rlist[NoSPRserpentD].x=306<<16
		rlist[NoSPRserpentD].y=40+8<<16
       ...
       IF GOTOU = 0 AND gnome = 1 THEN ' football avec le gnome */
          ...
          ' gnometapetete  */
           ...
           IF GOTOU = 0 AND football = 1 THEN
               ...
               IF GOTOU = 0 AND temps > reftempsB + 60 THEN
                   IF xGNOME < xA AND xGNOME < xB THEN
							IF musiqueLOOP = 0 THEN SNDZEROPLAY(1, strptr(sfx_tete), (strptr(sfx_tete_end)-strptr(sfx_tete)+3) and 0xfffffffc, 46168/9233, Zero_Audio_8bit_muLaw|0)
                   END IF
    ...
    IF GOTOU = 0 THEN
        ...
        IF jeu = 2 THEN RLOCATE 80,144: RPRINT "THANKS.BIG.BOY"

        IF jeu = 0 AND joueurA = 6660 THEN RLOCATE 88,128: RPRINT "YOUR.END.HAS.COME"
...
'RLOCATE 128,4: RPRINT "BETA.076"
'RLOCATE 100,16: RPRINT "   ":RPRINTINT xA 
'RLOCATE 160,16: RPRINT "   ":RPRINTINT yTA

' *** DEBUG *****	
if GOTOU = 0 THEN
	'RLOCATE 0,192: RPRINT "      ": RPRINTINT marianna
	'RLOCATE 0,200: RPRINT "      ": RPRINTINT xB
END IF	



' rajouter son boule (avoir)
' faire degats sorcier 

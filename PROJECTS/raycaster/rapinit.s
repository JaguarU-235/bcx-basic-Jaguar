...
;			value						possible values				description															rb+ label to access
	...
	dc.w	0,0				; 									; How many pixels will this sprite move automatically in x?			;   R_sprite_xadd
	dc.l	4				; 									; Width of sprite in pixels											; * R_sprite_width
	...
	dc.l	SPR_LINEB2	;									; Place in memory where the sprite lives: label from RAPAPP.S		;   R_sprite_gfxbase
	...
	dc.l	4*240*2		; 									; Size of sprite in bytes - used to find next frame of animation	;   R_sprite_framesz
																...
																; *2 for 16bit
																; *4 for 24bit
	dc.l	4*2			; 									; Width of a single row of sprite pixels							; * R_sprite_bytewid
	...
	dc.l	4*2			;									; Width of sprite within larger graphic.							;   R_sprite_gwidth
																; Enter the x-size and add the following:
																; /8 for 1bit
																; /4 for 2bit
																; /2 for 4bit
...
; ROAD LINES
;			value						possible values				description															rb+ label to access
	dc.l	80				; 									; How many sprites like this one do you want in the sprite list?
	...
	dc.l	0			;									; Width of collision box from centre in pixels						; * R_sprite_hbox
	dc.l	0			;									; Height of collision box from centre in pixels						; * R_sprite_vbox
	dc.l	SPR_LINEB1	;									; Place in memory where the sprite lives: label from RAPAPP.S		;   R_sprite_gfxbase
	dc.l	16				; 1,  2,  4,  8,  16,  24			; Sprite graphics type: 1,2,4,8,16 or 24 bit						; * R_sprite_?
	dc.l	is_RGB			; is_RGB			is_cry			; Sprite graphics type. RGB or Atari's CRY format.					; * R_sprite_?
	dc.l	is_opaque		; is_trans			is_opaque		; Trans for see-through background, opaque for solid background		; * R_sprite_?
	dc.l	4*240*2		; 									; Size of sprite in bytes - used to find next frame of animation	;   R_sprite_framesz
